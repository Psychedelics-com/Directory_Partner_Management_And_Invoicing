 ...
 */
router.post('/bookings', ensureAuthenticated, async (req, res) => {
    try {
        const { partner_id, guest_name, retreat_date, status, expected_net_revenue, final_net_revenue } = req.body;

        if (!partner_id || !guest_name || !retreat_date || !status) {
            return res.status(400).json({ error: 'Partner ID, guest name, retreat date, and status are required' });
        }

        const result = await query(
            `INSERT INTO retreat_bookings (partner_id, guest_name, retreat_date, status, expected_net_revenue, final_net_revenue)
             VALUES ($1, $2, $3, $4, $5, $6)
             RETURNING *`,
            [partner_id, guest_name, retreat_date, status, expected_net_revenue || null, final_net_revenue || null]
        );
 ...
 */
router.put('/bookings/:id', ensureAuthenticated, async (req, res) => {
    try {
        const { id } = req.params;
        const { guest_name, retreat_date, status, expected_net_revenue, final_net_revenue } = req.body;

        const result = await query(
            `UPDATE retreat_bookings 
             SET guest_name = $1, retreat_date = $2, status = $3, 
                 expected_net_revenue = $4, final_net_revenue = $5
             WHERE id = $6
             RETURNING *`,
            [guest_name, retreat_date, status, expected_net_revenue || null, final_net_revenue || null, id]
        );
 ...
 * Delete a booking
 */
router.delete('/bookings/:id', ensureAuthenticated, async (req, res) => {
    try {
        const { id } = req.params;

        const result = await query('DELETE FROM retreat_bookings WHERE id = $1 RETURNING *', [id]);
